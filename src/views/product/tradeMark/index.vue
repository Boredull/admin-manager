<template>
  <div>
    ...
    <!-- 
      ...
      Form 组件提供了表单验证的功能，只需要通过rules属性传入约定的验证规则，并将Form-Item的prop属性设置为需校验的字段名即可
     -->
    <el-dialog
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        // 品牌名称的验证规则
        // required:必须要校验字段（前面五角星有关系） message 提示信息  trigger：用户行为设置（事件的设置:blur失焦、change:文本发生变化）
        tmName: [
